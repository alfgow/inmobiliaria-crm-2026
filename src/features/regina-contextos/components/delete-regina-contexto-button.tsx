...
import { deleteReginaContexto } from "@/features/regina-contextos/actions/delete-regina-contexto";
...
export function DeleteReginaContextoButton({ waId, label }: Props) {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const result = await deleteReginaContexto(waId);
  ...
  return (
    <>
      <button
        ...
        disabled={isPending}
        className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-white px-3.5 py-2 text-sm font-medium text-rose-600 shadow-[0_12px_30px_rgba(15,23,42,0.06)] transition hover:-translate-y-0.5 hover:border-rose-300 hover:text-rose-700 disabled:pointer-events-none disabled:opacity-60"
      >
        {isPending ? (
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-2 text-sm leading-6 text-white/70">
                    Vas a eliminar el contexto{" "}
                    <span className="font-semibold text-white">{waId}</span>.
                  </p>
                </div>
                <button
                  type="button"
